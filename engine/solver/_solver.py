"""
Copied from DEIMv2 (https://github.com/Intellindust-AI-Lab/DEIMv2)
...
class BaseSolver(object):
    ...
    #
    #     # 确定要加载的 state_dict：优先用 'ema'，其次 'model'，再其次整个 ckpt
    #     if 'ema' in state:
    #         sd = state['ema']
    ...
    #     else:
    #         sd = state  # assume ckpt is raw state_dict
    ...
    #
    #     # 加载到 self.model（这才是 test 时 forward 用的！）
    #     if self.model is not None:
    #         missing, unexpected = self.model.load_state_dict(sd, strict=False)
    #         print(f"  => Missing keys: {len(missing)}")
    #         print(f"  => Unexpected keys: {len(unexpected)}")
    #         if len(missing) > 0:
    #             print("  => Some layers were NOT loaded (e.g., head mismatch?)")
    #     else:
    #         print("  => self.model is None, skip loading.")
